...
const UpdateUser = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${apiConfig.baseURL}${apiConfig.endpoints.ressourcesAll}`);
...
const handleSaveEdit = async () => {
  try {
    ...
    const response = await axios.put(
      `${apiConfig.baseURL}${apiConfig.endpoints.updateRessource(originalData["personalid"])}`,
      updateData,
      {
        headers: {
  ...
  const handleDeleteRow = async (row) => {
    ...
    try {
      await axios.delete(
        `${apiConfig.baseURL}${apiConfig.endpoints.deleteRessource(row["personalid"])}`
      );
      fetchData();
  ...
  return (
    <div style={{ display: "flex", gap: 0, margin: 0, padding: 0, width: "100%", minHeight: "100vh" }}>
      <div
        style={{
          flexGrow: 1,
          width: "100%",
